...
the native linux ``KVM`` through ``libvirt``. Vagrat  unfortunately doesn't support
...
installlation guide](https://github.com/vagrant-libvirt/vagrant-libvirt)
...
| wortker-01 	| 192.168.50.11        	|
| wortker-02 	| 192.168.50.12        	|
| ...     	    |                      	|
| worker-XX 	| 192.168.50.(10 + XX) 	|


For **cluster management** we will use the Spark in the simple **Standalone**
mode. To reade more about cluster management options, please refer to the
...
For our purpuses, we will setup our virtual machines using ``vagrant``. We
...
wish to permanantly delete your machines it use ``vagrant destroy``.
...
hosts into **groups**, so you can run related task for multiple nodes at onece,
...
able to autogenerate the inventory file for us. After running ``vagrant up``
with probisioning using Ansible set up, you can find the autogenerated file in
...
If you wish to know more about crating invetories, please refer to [How to
...
to use the Vagrant generated invetory.
...
Let's disect the example playbook from Vagrant configuration.
```yaml
--- # Three dashes signify YAML file, Ansible doesn't care if you forget them though
- name: My first play.
  hosts: all
...
The ``become`` option allows us to elevate the play to superuser privilages.
...
Here we can see we can get superuser privilages for a single task only.
...
option. For the sake of brevity we declared the following variables (which we wil use
...
To configure spark enviroment, we will want to define the following
...
We will look at two ways of simplyfing playbooks, namely **roles** and
...
To simplify our playbook, we might want to define two roles:
    - ``common``: role that setups common enevironmanet on all host
    - ``spark``: role that installs Spark

We therfore create the following directory structure:
```
 roles
 ├── common
 │   └── tasks
...
Basic overview of the directories:
 - ``defaults``: Contains default variables or variables we expect will be overwritted
